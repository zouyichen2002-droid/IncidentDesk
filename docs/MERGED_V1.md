...
| 模块 | 职责 |
|---|---|
| Go 业务控制层 | 登录认证、权限复验、持久任务与历史、幂等请求、审批与动作边界 |
| Python 问数执行层 | FastAPI + LangGraph 查询流程、Qdrant 字段和指标检索、Elasticsearch 字段值检索、MySQL 查询与纠错 |
| React 工作台 | 自然语言入口、意图分流、阶段进度、结果表格、SQL 与证据来源 |
| Python 调查执行层 | 日志、发布、Git 与运行手册检索，证据组织与调查状态恢复 |

问数代码位于 `query/`，与业务控制层使用同一仓库和部署入口。
...
两个 Python 服务暂时独立运行：旧调查 Worker 使用 Python 3.12，问数模块使用 Python 3.14。它们属于同一个仓库、部署与产品，不强行合并不兼容的依赖环境。
